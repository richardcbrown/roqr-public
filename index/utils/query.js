...
var query = 
{
    ...
    filters:
    {
        _baseFilter(documents, parameter) {
            ...
            try
            {
                selector = this.selectors[parameter.global].call(this, parameter).paths[0].split(',');
            }
            catch(ex) 
            {
                throw 'Could not execute node path selector for global type: ' + parameter.global + ', reason: ' + ex.stack || ex.toString();
            }
        ...
        token: function(documents, parameter) {
            ...
            try
            {
                selector = this.selectors[parameter.global].call(this, parameter);
            }
            catch(ex) 
            {
                throw 'Could not execute node path selector for global type: ' + parameter.global + ', reason: ' + ex.stack || ex.toString();
        ...
        _select: function(selector, documents, parameter) {
            ...
            documents.$(selector).forEachChild(function(value,node) {
                ...
                paramValues.forEach(function(paramValue) {
                    var match = false;
                    var modifier = parameter.modifier || parameter.global;
                    try
                    {
    ...
    modifiers:
    {
        ...
        identifier:function(value,expression) {
            //Reference should also work like token searches...
        },
